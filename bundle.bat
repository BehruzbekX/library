...
:: Check if the root directory exists
    ...
    echo Running command: oj-bundle !includeDirs! maint.test.cpp > temp.test.cpp 2> error.log

    :: Run oj-bundle with the collected paths
    oj-bundle !includeDirs! main.test.cpp > temp.test.cpp 2> error.log
    ...
    if exist "temp.test.cpp" (
        setlocal enabledelayedexpansion
        set /a count=0
        for /f "usebackq tokens=*" %%A in ("temp.test..cpp") do (
            set /a count+=1
            if !count! geq 20 (
                goto :break
            )
